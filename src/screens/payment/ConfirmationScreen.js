...
export default function ConfirmationScreen({ navigation, route }) {
  ...
  // Polling quand paiement PENDING
  useEffect(() => {
    if (!pendingRef) return;
    let elapsed = 0;
    const TIMEOUT = 180; // 3 minutes max
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (elapsed >= TIMEOUT) {
        ...
        setPendingRef(null);
        Alert.alert('Délai dépassé', 'Aucune confirmation reçue. Vérifiez votre téléphone.');
        return;
      }
      try {
        const res = await paymentService.checkStatus(token, pendingRef);
        if (res.status === 'SUCCESSFUL') {
          ...
          clearInterval(pollRef.current);
          setPendingRef(null);
          Alert.alert('Paiement refusé', 'La transaction a été refusée par l\'opérateur.');
        }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (result.statut === 'SUCCESSFUL') {
        ...
        Alert.alert('Paiement refusé', 'La transaction a été refusée.');
      }
    } catch (e) {
  ...
  if (pendingRef) {
    const operator = params.methode === 'ORANGE_MONEY' ? 'Orange Money' : 'MTN MoMo';
    const remaining = Math.max(0, 180 - pollSeconds);
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.pendingContainer}>
          ...
          <Text style={styles.pendingSub}>
            Vérifiez votre téléphone{'\n'}et confirmez le paiement {operator}
          </Text>
          <View style={styles.pendingCard}>
            <Text style={styles.pendingAmount}>{formatFCFA(montant)}</Text>
            <Text style={styles.pendingMerchant}>{params.merchantName}</Text>
            <Text style={styles.pendingPhone}>{params.phoneFrom}</Text>
          </View>
          <Text style={styles.pendingTimer}>
            Expiration dans {Math.floor(remaining / 60)}:{String(remaining % 60).padStart(2, '0')}
          </Text>
          <TouchableOpacity onPress={() => { clearInterval(pollRef.current); setPendingRef(null); }}>
            <Text style={styles.pendingCancel}>Annuler</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
...
const styles = StyleSheet.create({
  ...
  pendingContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 16, padding: Layout.screenPaddingHorizontal },
  pendingTitle: { fontSize: 22, fontWeight: '700', color: Colors.textPrimary, textAlign: 'center' },
  pendingSub: { fontSize: 14, color: Colors.textSecondary, textAlign: 'center', lineHeight: 22 },
  pendingCard: { backgroundColor: Colors.accent, borderRadius: 16, padding: 20, alignItems: 'center', gap: 4, width: '100%' },
  pendingAmount: { fontSize: 32, fontWeight: '700', color: Colors.primary },
  pendingMerchant: { fontSize: 15, fontWeight: '600', color: Colors.textPrimary },
  pendingPhone: { fontSize: 13, color: Colors.textSecondary },
  pendingTimer: { fontSize: 13, color: Colors.textSecondary },
  pendingCancel: { fontSize: 14, color: Colors.danger, fontWeight: '500' },
  btn: {
